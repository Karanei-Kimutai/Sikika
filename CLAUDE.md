...
- **Cloudinary**: evidence files, support resources, and legal case PDFs all use Cloudinary. All three use `type: authenticated` (private). Evidence and legal docs are accessed via short-lived signed URLs; support resources are streamed through the backend proxy (`GET /api/resources/:resourceId/file`) so authenticated Cloudinary URLs never reach the browser — the library remains publicly readable because the proxy route is unauthenticated. Without env vars, upload and streaming endpoints return 503 but list endpoints still work (fallback data shown in the Library).
